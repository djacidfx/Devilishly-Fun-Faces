...
public class TouchImageView extends AppCompatImageView {
    ...
    private class DoubleTapZoom implements Runnable {
        ...
        private double calculateDeltaScale(float f) {
            ...
            return (f2 + ((this.targetZoom - f2) * f)) / TouchImageView.this.normalizedScale;
        }
    }
                ...
                if (action != 1) {
                    if (action == 2) {
                        if (this.state != State.NONE) {
                            ...
                            getFixDragTrans(f, this.viewWidth, getImageWidth());
                            getFixDragTrans(f2, this.viewHeight, getImageHeight());
